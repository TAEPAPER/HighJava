...
public class EnumTest {  
	...
	/*
		enum(열거형)==> 서로 관련있는 상수들의 집합을 나타낸다.
		    	  ...
		    	  ==> 열거형은 클래스처럼 독립된 java파일로 만들 수 있고,
		    	       하나의 Java 파일에 클래스와 같이 만들 수 있고, 클래스 안에 내부 클래스처럼 만들 수 있다.
        ...
        -열거형의 속성 및 메서드
         ...
         - 열거형 선언하기
         방법1) 
         enum 열거형이름 {상수명1, 상수명2,...., 상수명 n }; 
         
         방법2) ==> 상수에 임의값을 지정하는 방법
         enum 열거형이름{
             상수명1(값들...),상수명2(값들...),..,상수명 n(값들...);   //마지막 상수을 지정한 후에 세미콜론을 붙인다.
             ...
             //열거형 생성자의 접근제한자는 묵시적으로 private이다.(생략가능)
             //변수명의 개수는 '값들'의 개수와 같아야하고 각 변수들의 자료형도 '값들'의 자료형과 같아야한다.
             private 열거형이름(자료형이름 변수명1,...){
             	위에 선언된 변수들을 초기화하는 작업 진행
             	...
             }
             
             // 위에 선언된 변수의 값을 외부에서 불러서 사용할 수 있도록 getter메서드를 만든다(접근제한자는 public으로함)
              public 자료형이름 get변수명(){
                  return 변수명;
              }     
	...
	*/
	...
	public static void main(String[] args) {
		...
		System.out.println("GREEN  : "+ ConstTest.GREEN);
		...
		System.out.println("mycol의 ordinary :"+ mycol.ordinal());
		System.out.println("mycnt의 ordinary :"+ mycnt.ordinal());
		System.out.println("----------------------------------");
		/*
		 * 서로 다른 종류의 열거형끼리의 비교는 불가하다.(못하게 만들어짐)
		 * 
		if(mycol == mycnt) {
			System.out.println("같다");
		}else {

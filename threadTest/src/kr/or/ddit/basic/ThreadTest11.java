...
public class ThreadTest11 {
	...
	public static void main(String[] args) {
		/*
		 * ThreadStopTest01 th1 = new ThreadStopTest01(); th1.start(); try {
		 * Thread.sleep(1000); } catch (InterruptedException e) { // TODO: handle
		 * exception } // th1.stop(); ==>느닷없이 멈춤(자원정리,쓰레드종료 출력x) th1.setStop(true);
		 */
		...
		th2.interrupt(); // 이걸쓰면 InterruptedException e 오류발생(?) 으아 잘 모르겠다 ㅜㅜ
...
class ThreadStopTest02 extends Thread {
	...
	public void run() {
		 ...
		 * System.out.println("실행 중..."); Thread.sleep(1); } }catch(InterruptedException
		 * e){ System.out.println("interrupt 발생"); }
		 */
		...
		while (true) {
			 ...
			 * 
			 * //검사방법1 ==> 쓰레드의 인스턴스 메서드인 isInterrupted()를 이용하기 // isInterrupted()메서드는
			 * interrupt()메서드가 호출되면 true를 반환한다. if(this.isInterrupted()) { break; }
			 */

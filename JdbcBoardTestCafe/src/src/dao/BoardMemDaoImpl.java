...
public class BoardMemDaoImpl implements BoardMemDao {
	...
	public List<BoardMemVO> choiceBoard(Connection conn,int boardNo ) throws SQLException {
		 String sql = "select jdbc_no,board_title,board_writer, to_char(board_date,''yyyy-mm-dd') board_date,board_cnt, board_ content from jdbc_board where board_no = ?";
		 conn = DBUtil3.getConnection();
		   PreparedStatement  pstmt= conn.prepareStatement(sql);
	...
	public List<BoardMemVO> titleSearch(Connection conn, String searching) {
		...
		public int setCountIncrement(Connection conn, int boardNo)

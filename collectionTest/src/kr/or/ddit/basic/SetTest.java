...
public class SetTest {
	...
	public static void main(String[] args) {
		 ...
		 */
		  ...
		  System.out.println("hs1 => " + hs1);  //추가한 순서랑 다르다
		...
		//삭제하는 메서드 : remove(삭제할 데이터) //데이터를 직접 찾아서 삭제 (인덱스 remove는 없음)
		...
		// System.out.println("hs1 =>"+ hs1);
		 ...
		 /*
		  Set의 데이터는 순서(index)가 없기 때문에 List처럼 index를 이용해서 데이터를
		 하나씩 불러올 수 없다.
		 그래서 **데이터를 하나씩 얻기 위해서는 Iterator형 객체로 변환**해야 한다.
		...
		/*Iterator의 hasNext()메서드
		   ==>Iterator에서 현재 데이터를 가리키는 포인터가 있는데 현재 포인터가 가리키는 곳의 
		   다음 번째 자리에 데이터가 있는지 검사한다
		   데이터가 있으면 true, 없으면 false를 반환한다.
		 */
		while(it.hasNext()) {
			/*Iterator의 next()메서드 => Iterator의 포인터를 다음번째로 이동시킨 후
			  이동한 자리의 데이터를 읽어와 반환한다
			 */ 
		...
		while(testSet.size()<3) {
			...
			testSet.add(rnd); //set에 이미 있으면 안들어가짐
		}

 ...
 *  2)DatagramPacket : 주고 받는 데이터와 관련된 작업을 수행한다.(편지 ,소포)
 *                     ==> 송신용 위한 생성자와 수신을 위한 생성자를 따로 제공한다.
 *     
 *  			
 * 
 */
	...
	public static void main(String[] args) {
		try {
			...
			while(true) {
				...
				//수신용 패킷 객체 생성 ==> 데이터가 저장될 byte형 배열, 배열의 길이를 이용하여 DatagramPacket객체를 생성한다.
				inpacket = new DatagramPacket(bMsg, bMsg.length);
				//데이터 수신하기 ==>receive()메서드 이용
				//            ==>receive()메서드는 데이터가 올 때까지 기다린다.
				//			     수신된 데이터의 패킷정보는 지정한 패킷객체변수에 저장된다.
				socket.receive(inpacket);
				//수신받은 패킷에서 상대방의 주소, 프토번호등을 알 수 있다.
				...
				//byte형 배열의 데이터를 문자열로 변환하기    //0번째 부터 getLength()만큼 다 문자열고 바꿔라
				...
				//송신용 패킷객체 생성 ==> 
				//전송할 데이터가 저장된 바이트 배열 , 전송할 자료의 길이(배열의 길이) ,상대방주소정보(IP), 포트번호를 지정하여 DatagramPacket객체를 생성 [총 4가지 필요] 
				outpacket = new DatagramPacket(sendMsg, sendMsg.length, address, port);
				
				//데이터 송신하기 ==> send()메서드를 사용한다 ==>send()메서드에 지정한 패킷객체를 전송한다.
